 ...
 public boolean insertarVertice(TVertice vertice) {
    ...
    public void desvisitarVertices() {
        for(Comparable key : vertices.keySet()){
            vertices.get(key).setVisitado(false);
        }
    }
    ...
    public boolean tieneCiclo() {
        ...
        for(Comparable key : this.getVertices().keySet()){
            TVertice vert = this.getVertices().get(key);
            if(!vert.getVisitado()){
                TCamino tecamino = new TCamino(vert);
                boolean tieneCiclo = vert.tieneCiclo(tecamino);
                if(tieneCiclo){
                    return true;
                }
            }
    ...
    public TCamino caminoCritico(Comparable origen, Comparable destino){        
        if(!this.tieneCiclo()){            
            TCaminos caminosPosibles = this.todosLosCaminos(origen,destino);
            Double maximo=0.0d;            
            TCamino maxActual = null;
            
            //Busco maximo
            for(TCamino cam : caminosPosibles.getCaminos()){
                if(cam.getCostoTotal()>maximo){
                    maximo=cam.getCostoTotal();
                    maxActual=cam;
                }                     
            }
    ...
    public Map<TCamino,Double> holgura(Comparable origen, Comparable destino){
        if(!this.tieneCiclo()){            
            TCaminos caminosPosibles = this.todosLosCaminos(origen,destino);
            Double maximo=0.0d;            
            TCamino maxActual = null;
            ...
            Map<TCamino,Double> holguras = new LinkedHashMap<TCamino,Double>();
            for(TCamino cam : caminosPosibles.getCaminos()){
                holguras.put(cam,maximo-cam.getCostoTotal());
            }
